...
RSpec.describe User, type: :model do
  ...
  describe 'ユーザー新規登録' do
    it 'ニックネーム、メールアドレス、パスワード、パスワード（確認）、ユーザー本名、ユーザー本名カナ、生年月日が存在すれば登録できること' do
      @user.password = '55555a'
      @user.password_confirmation = '55555a'
    ...
    it 'ユーザー本名のフリガナは全角カタカナでなければ登録できないこと' do
      @user.last_name_kana = @user.last_name
      @user.first_name_kana = @user.first_name
      @user.valid?
      expect(@user.errors.full_messages).to include('First name kana 全角カタカナで入力してください', 'Last name kana 全角カタカナで入力してください')
    end
